...
| `./event-bridge` | `ensureEventBridgeRule(read, write, rule, opts?)`: identity-aware (`pattern` + `subjectPrefix`) merge-not-clobber upsert into an `rt.notify.eventBridges`-shaped list; `deckAppUrl(name, fallback, opts?)`: an app's local url from deck's `/api/v1/status`, falling back on any failure | yes                     |
| `./shell-handoff` | `shellHandoff(req, deps?)`: hands a top-level `.mattstack` document request to the mattstack window; returns the stub `Response` when handled, `null` to serve the app normally (fails open on every miss) | yes                     |
