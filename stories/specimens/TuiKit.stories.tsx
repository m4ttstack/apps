  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {SURFACES.map(surface => (
        <div
          key={surface}
          style={{ background: `var(${surface})`, padding: 16, borderRadius: 8, display: 'flex', gap: 16, flexWrap: 'wrap', fontFamily: 'var(--font-sans)' }}
        >
          {HUES.map(hue => (
            <span key={hue} style={{ color: `var(--text-${hue}-vivid)`, fontSize: 13 }}>
