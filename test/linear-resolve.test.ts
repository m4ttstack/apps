...
describe("resolveLinearTickets resilience", () => {
  ...
  it("falls back to individual lookups when a known-valid batch chunk fails", async () => {
    getStore().putLinearIds([{ id: "CV-9001", valid: true }, { id: "CV-9002", valid: true }]);
    ...
    expect(issues.map((i) => i.identifier).sort()).toEqual(["CV-9001", "CV-9002"]);
    ...
    expect(queries.filter((ids) => ids.length > 1)).toEqual([["CV-9001", "CV-9002"]]);
    expect(calls.filter((ids) => ids.length === 1).flat().sort()).toEqual(["CV-9001", "CV-9002"]);
  });
  ...
  it("warns when identifiers are lost even after the individual fallback", async () => {
    ...
    const warnings: LeaderboardWarning[] = [];
    const issues = await resolveLinearTickets("key", [sourceMr(1, "CV-9010")], warnings);
  ...
  it("caches an identifier as invalid on an entity-not-found error, without warning", async () => {
    ...
    const warnings: LeaderboardWarning[] = [];
    const issues = await resolveLinearTickets("key", [sourceMr(1, "CV-9040")], warnings);
  ...
  it("still records definitive answers from a successful verify", async () => {
    stubLinear((ids) => okData(ids, (id) => (id === "CV-9030" ? rawFor(id) : null)));

    const warnings: LeaderboardWarning[] = [];
    await resolveLinearTickets("key", [sourceMr(1, "CV-9030"), sourceMr(2, "CV-9031")], warnings);
...
describe("resolveLinearTickets credit rule", () => {
  const linkedMr = (m: Partial<NormMr> & Pick<NormMr, "iid" | "authorUsername">) =>
    mr({ title: "CV-9100: do the thing", ...m });

  const resolveCredit = async (mrs: NormMr[], roster: readonly string[] = []) => {
  ...
  it("credits the merged MR's author over an open one, regardless of scan order", async () => {
    const merged = linkedMr({ iid: 1, authorUsername: "alice", state: "merged", mergedAt: "2026-05-02T00:00:00.000Z" });
    const open = linkedMr({ iid: 2, authorUsername: "bob", state: "opened", mergedAt: null });
  ...
  it("credits the earlier of two merged MRs, not the later one", async () => {
    const earlier = linkedMr({ iid: 1, authorUsername: "alice", state: "merged", mergedAt: "2026-05-01T00:00:00.000Z" });
    const later = linkedMr({ iid: 2, authorUsername: "bob", state: "merged", mergedAt: "2026-05-05T00:00:00.000Z" });
  ...
  it("prefers a roster author's merged MR over a non-roster author's, even when it merged later", async () => {
    const nonRoster = linkedMr({ iid: 1, authorUsername: "outsider", state: "merged", mergedAt: "2026-05-01T00:00:00.000Z" });
    const roster = linkedMr({ iid: 2, authorUsername: "alice", state: "merged", mergedAt: "2026-05-05T00:00:00.000Z" });
  ...
  it("leaves assignedUser null when nothing merged and several distinct authors are linked", async () => {
    const a = linkedMr({ iid: 1, authorUsername: "alice", state: "opened", mergedAt: null });
    const b = linkedMr({ iid: 2, authorUsername: "bob", state: "opened", mergedAt: null });
  ...
  it("credits the sole author when nothing merged but every linked MR shares one author", async () => {
    const a = linkedMr({ iid: 1, authorUsername: "alice", state: "opened", mergedAt: null });
    const b = linkedMr({ iid: 2, authorUsername: "alice", state: "opened", mergedAt: null });
  ...
  it("breaks a tie between merged MRs with identical mergedAt by the lower (projectPath, iid)", async () => {
    ...
    const higher = linkedMr({ iid: 5, projectPath: "org/app", authorUsername: "bob", state: "merged", mergedAt: sameTime });
    const lower = linkedMr({ iid: 2, projectPath: "org/app", authorUsername: "alice", state: "merged", mergedAt: sameTime });
