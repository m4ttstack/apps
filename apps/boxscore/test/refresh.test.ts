...
function makeFakeProvider(overrides: FakeOverrides = {}) {
  ...
  const provider: SourceProvider = {
    async fetchMergeRequestIndex(options) {
      ...
      return overrides.fetchMergeRequestIndex
        ? await overrides.fetchMergeRequestIndex(options)
        : [];
    },
    async fetchMergeRequestMetrics(projectPath, mrIid, options) {
      calls.push({
        method: 'fetchMergeRequestMetrics',
        args: [projectPath, mrIid, options],
      });
    ...
    async fetchProjectPipelines(projectPath, options) {
      calls.push({
        ...
        args: [projectPath, options],
      });
      return overrides.fetchProjectPipelines
        ? await overrides.fetchProjectPipelines(projectPath, options)
        : [];
    },
    async fetchUserEvents(userId, options) {
      calls.push({ method: 'fetchUserEvents', args: [userId, options] });
      return overrides.fetchUserEvents
        ? await overrides.fetchUserEvents(userId, options)
        : [];
    },
...
const indexRow = (
  over: Partial<MergeRequestIndexRow> = {}
): MergeRequestIndexRow => ({
  iid: 1,
  projectPath: 'g/p',
  title: 't',
...
const metrics = (
  over: Partial<MergeRequestMetrics> &
    Pick<MergeRequestMetrics, 'projectPath' | 'iid'>
): MergeRequestMetrics => ({
  description: 'desc',
...
const userIdRes = (id: number, username: string) =>
  new Response(JSON.stringify([{ id, username, name: username }]), {
    status: 200,
  });
  ...
  it('resolves only roster users missing from identities or older than a day; a fresh one is not re-fetched', async () => {
    ...
    const { provider, calls } = makeFakeProvider({
      restRequest: async (_m, path) => {
        const username = new URL(path, 'http://x').searchParams.get(
          'username'
        )!;
    ...
    const lookedUp = calls
      ...
      .map(c =>
        new URL(c.args[1] as string, 'http://x').searchParams.get('username')
      );
  ...
  it('scans/resolves for hidden members too, so hiding a user does not evict their data', async () => {
    ...
    const { provider, calls } = makeFakeProvider({
      ...
      fetchMergeRequestIndex: async () => [
        indexRow({ iid: 1, authorUsername: 'hidden' }),
      ],
    });
    ...
    const detailCalls = calls.filter(
      c => c.method === 'fetchMergeRequestMetrics'
    );
...
describe('runRefresh: watermark-driven project scan', () => {
  ...
  it('backfills from the window start when it precedes the floor, and lowers the floor to it', async () => {
    ...
    const scan = calls.find(c => c.method === 'fetchMergeRequestIndex')!
      .args[0] as FetchMergeRequestIndexOptions;
...
describe('runRefresh: failed-scan invariant', () => {
  it("leaves the failed project's watermark untouched, advances the other, and warns instead of throwing", async () => {
    const store = getStore();
    store.recordScan('g/A', {
      from: '2026-03-01T00:00:00.000Z',
      at: '2026-04-01T00:00:00.000Z',
    });
    store.recordScan('g/B', {
      from: '2026-03-01T00:00:00.000Z',
      at: '2026-04-01T00:00:00.000Z',
    });
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'mr_fetch_failed' })
    );
  });
...
describe('runRefresh: metrics batch persistence', () => {
  it('persists in batches of 25', async () => {
    ...
    store.upsertIndexRows(
      rows.map(r => toIndexRow(r, '2026-05-01T00:00:00.000Z'))
    );
    const { provider } = makeFakeProvider({
      fetchMergeRequestMetrics: async (_p, iid) =>
        metrics({ projectPath: 'g/p', iid }),
    ...
    expect(
      store.metricsByKeys(rows.map(r => mrKey(r.projectPath, r.iid)))
    ).toHaveLength(30);
  });
  ...
  it('banks completed batches when the run is aborted mid-flight', async () => {
    ...
    const stored = store.metricsByKeys(
      rows.map(r => mrKey(r.projectPath, r.iid))
    );
...
describe('runRefresh: warning codes', () => {
  it('emits user_unresolved when a username has no match', async () => {
    const store = getStore();
    const { provider } = makeFakeProvider({
      restRequest: async () => new Response('[]', { status: 200 }),
    });
    const warnings = await runRefresh({
      store,
      provider,
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'user_unresolved' })
    );
  });
  ...
  it('emits user_lookup_failed when identity resolution throws', async () => {
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'user_lookup_failed' })
    );
  });
  ...
  it('emits mr_detail_partial when a metrics fetch throws', async () => {
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'mr_detail_partial' })
    );
  });
  ...
  it('emits pipeline_fetch_failed when the pipeline fetch throws', async () => {
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'pipeline_fetch_failed' })
    );
  });
  ...
  it('emits events_fetch_failed when the push-events fetch throws', async () => {
    ...
    expect(warnings).toContainEqual(
      expect.objectContaining({ code: 'events_fetch_failed' })
    );
  });
});
...
describe('runRefresh: push events stay project-scoped', () => {
  it('stores only the push event to a configured project; the out-of-scope one is dropped', async () => {
      ...
      fetchUserEvents: async () => [
        {
          ...
          repositoryId: 'gitlab:g/p',
        },
        {
          action: 'pushed',
          createdAt: '2026-05-11T00:00:00.000Z',
          repositoryId: 'gitlab:other/repo',
        },
      ],
    });
    ...
    const stored = store.pushEventsBetween(
