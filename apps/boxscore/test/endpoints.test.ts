...
describe('refresh endpoints', () => {
  ...
  it('POST /api/refresh/:id/cancel cancels a running job', async () => {
    ...
    const res = await app.request(`/api/refresh/${job.id}/cancel`, {
      method: 'POST',
    });
    expect(res.status).toBe(200);
    await flush();
    const after = (await Promise.resolve(
      app.request(`/api/refresh/${job.id}`)
  ...
  it('POST /api/refresh rejects an invalid custom range with 400', async () => {
    const res = await app.request(
      '/api/refresh?range=custom&start=2026-05-01T00:00:00.000Z',
      { method: 'POST' }
    );
  ...
  it("GET /api/leaderboard?cacheOnly=1 serves a snapshot without refetching once every configured project's floor reaches the window start", async () => {
    ...
    for (const p of PROJECTS) {
      getStore().recordScan(p, {
        from: new Date(now - 100 * DAY_MS).toISOString(),
  ...
  it('GET /api/leaderboard?cacheOnly=1 returns {cached:false} when the scan floor is later than the window start', async () => {
    ...
    for (const p of PROJECTS) {
      getStore().recordScan(p, {
        from: new Date(now - 3 * DAY_MS).toISOString(),
        at: new Date(now).toISOString(),
      });
    }
    const res = await app.request('/api/leaderboard?range=7d&cacheOnly=1');
