...
vi.mock('@mattstack/settings-kit/react', () => ({
  useSettingsScope,
  useSettingKey,
}));

function def(
  overrides: Partial<SettingDefWire> & { key: string }
): SettingDefWire {
  return {
    type: 'array',
    scopes: ['team'],
...
function scopeState(
  defs: SettingDefWire[],
  overrides: Partial<ReturnType<typeof useSettingsScope>> = {}
) {
  return {
    defs,
    loading: false,
...
function keyState(
  rosterDef: SettingDefWire | null,
  overrides: Partial<ReturnType<typeof useSettingKey>> = {}
) {
  return {
    def: rosterDef,
    rows: [],
...
describe('SettingsPage', () => {
  ...
  it('renders a non-writable key read-only, with no editor control', () => {
    ...
    expect(
      screen.queryByRole('textbox', { name: 'boxscore.ignoredMrs' })
    ).not.toBeInTheDocument();
  });

  it('puts team keys (and the roster) in the Team section and user keys in the You section', () => {
    useSettingsScope.mockReturnValue(
      scopeState([PROJECTS_DEF, DEFAULT_RANGE_DEF])
    );
    useSettingKey.mockReturnValue(keyState(ROSTER_DEF));
  ...
  it('writes the whole roster array back (not a partial patch) when a member is added', () => {
    ...
    fireEvent.change(screen.getByLabelText('new roster member username'), {
  ...
  it('renders the default-range key as a select over the range presets', () => {
    ...
    expect(
      screen.getByRole('combobox', { name: 'boxscore.defaultRange' })
    ).toHaveValue('30d');
  });
});
