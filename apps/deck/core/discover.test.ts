...
const route = (port: number): PortlessRoute => ({
  hostname: `a${port}.localhost`,
  port,
});
...
test('bareName strips any configured TLD labels, greedily from the right', () => {
  ...
  expect(bareName('deck.mattstack.localhost', ['localhost'])).toBe(
    'deck.mattstack'
  );
});
...
<key>ProgramArguments</key><array><string>/opt/homebrew/bin/cloudflared</string></array>
    ...
    if (prevAgents === undefined) delete process.env.LOCAL_AGENTS_DIR;
    else process.env.LOCAL_AGENTS_DIR = prevAgents;
    if (prevPids === undefined) delete process.env.LOCAL_LAUNCHCTL_PIDS;
