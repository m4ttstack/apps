...
test('an app record with no icon ingested reports icon null', async () => {
  writeFileSync(
    process.env.LOCAL_APPS_ROUTES_PATH!,
    JSON.stringify([{ hostname: 'noicon.localhost', port: 11004 }])
  );
  putRecord({
    name: 'noicon',
    managedBy: 'rt',
...
const api = (path: string) =>
  fetch(`http://127.0.0.1:${PORT}${path}`, {
    headers: { 'x-forwarded-host': 'chat.mattstack' },
  });
...
test('GET /api/apps echoes an allowed mattstack origin in CORS', async () => {
  const res = await fetch(`http://127.0.0.1:${PORT}/api/apps`, {
    headers: { origin: 'https://chat.mattstack' },
  });
...
test('the icon route is CORS-allowed too', async () => {
  const res = await fetch(`http://127.0.0.1:${PORT}/api/apps/chat/icon`, {
    headers: { origin: 'https://console.mattstack' },
  });
  expect(res.headers.get('access-control-allow-origin')).toBe(
    'https://console.mattstack'
  );
});
