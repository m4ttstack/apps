...
beforeAll(() => {
  ...
  domainServer = startApi({
    ...
    edge: new FakeEdgeProxy(),
    port: DOMAIN_PORT,
    canaryPort: DOMAIN_PORT + 1,
    freshness: () => 'unknown',
    autoHeal: () => null,
    onRouteWrite: () => {},
    tunnel: new FakeTunnelDriver(domainCfDir),
    cloudflaredDir: domainCfDir,
    dns: new FakeCfDns(),
    resolveCloudflared: () => '/opt/homebrew/bin/cloudflared',
  });
  ...
  cfServer = startApi({
    ...
    edge: new FakeEdgeProxy(),
    port: CF_PORT,
    canaryPort: CF_PORT + 1,
    freshness: () => 'unknown',
    autoHeal: () => null,
    onRouteWrite: () => {},
    tunnel: new FakeTunnelDriver(),
    accessFetch: cannedAccessFetch,
    ...
    deckSecrets: {
      ...
      post: async () => ({
        ok: true,
        data: { cfApiToken: 'cf-tok', cfZoneId: 'z1' },
      }),
    },
  });
});
...
const devApi = (path: string, init?: RequestInit) =>
  fetch(`http://127.0.0.1:${DEV_PORT}${path}`, init);
const devPost = (
  path: string,
  body: unknown,
  headers: Record<string, string> = {}
...
test('caller header drives the 409; ?force=true is the escape hatch', async () => {
  await post(
    '/api/v1/apps',
    { name: 'g', command: ['x'], workingDirectory: '/tmp' },
    { 'x-local-caller': 'rt' }
  );
...
test('managed/reresolve answers 200 with the ok/restarted/unchanged/failed body shape', async () => {
  await post(
    '/api/v1/apps',
    { name: 'rr1', command: ['bun', 's.ts'], workingDirectory: '/tmp' },
    { 'x-local-caller': 'rt' }
  );
  ...
  writeFileSync(
    join(rrDir, 'mattstack.deck.json'),
    JSON.stringify({ name: 'rr1', dev: { start: 'bun s.ts' } })
  );
...
test('publish flips settings through the versioned path', async () => {
  writeFileSync(
    ...
    JSON.stringify([{ hostname: 'p1.localhost', port: 12000, pid: 0 }])
  );
...
test('mutations through a public host are forbidden', async () => {
  const res = await post(
    ...
    { 'x-forwarded-host': 'apps.example.dev' }
  );
...
test('a successful Cloudflare sync persists the rule and reports it back on the row', async () => {
  ...
  const cfApi = (path: string, init?: RequestInit) =>
    fetch(`http://127.0.0.1:${CF_PORT}${path}`, init);
  const putJson = (path: string, payload: unknown) =>
    cfApi(path, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
  ...
  expect(
    cfCalls.some(c => c.method === 'POST' && c.url.endsWith('/access/apps'))
  ).toBe(true);
  expect(cfCalls.some(c => c.url.includes('/access/apps/app-1/policies'))).toBe(
    true
  );
...
test('legacy POST endpoints are gone (board speaks /api/v1 now)', async () => {
  for (const path of [
    ...
    const res = await api(path, {
      method: 'POST',
      body: new URLSearchParams({ app: 'x' }),
    });
...
test('POST /api/v1/migrate with convert:true skips a non-service (external) record without touching the manager', async () => {
  ...
  expect(
    [...manager.installed.keys()].some(label => label.includes('static-app'))
  ).toBe(false);
});

test('domain flow: bind, show reports identity + edge, unbind refuses served apps then forces', async () => {
  const domainApi = (path: string, init?: RequestInit) =>
    fetch(`http://127.0.0.1:${DOMAIN_PORT}${path}`, init);
  ...
  const bind = await domainPost('/api/v1/domain/bind', {
    domain: 'example.dev',
  });
  ...
  await domainPost('/api/v1/apps', {
    name: 'domain-flow-served',
    staticPort: 41999,
  });
...
test('DELETE tears down a route-only row through the edge driver instead of 404ing', async () => {
  ...
  expect(
    (await api('/api/v1/apps/neither-route-nor-record', { method: 'DELETE' }))
      .status
  ).toBe(404);
});
...
test('POST /apps/:name/alt activates and clears an overlay', async () => {
  ...
  expect((await (await api('/api/v1/apps/altapp')).json()).record.port).toBe(
    4500
  );
...
test('alt on an unknown app is 404', async () => {
  expect((await post('/api/v1/apps/ghost/alt', { alt: 'dev' })).status).toBe(
    404
  );
});
...
test('command route runs a declared command in dev', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: 'cmdapp',
      port: 4800,
      commands: { start: 's', build: 'echo built' },
    })
  );
...
test('an inherited Object.prototype command name is 404 in dev, not a silent match', async () => {
  ...
  await devPost('/api/v1/apps/register', { dir });
  expect(
    (await devPost('/api/v1/apps/cmdapp3/commands/constructor', {})).status
  ).toBe(404);
});
...
test('unknown command name is 404 in dev', async () => {
  ...
  writeFileSync(
    join(dir, 'mattstack.deck.json'),
    JSON.stringify({ name: 'cmdapp2', port: 4801, commands: { start: 's' } })
  );
  await devPost('/api/v1/apps/register', { dir });
  expect(
    (await devPost('/api/v1/apps/cmdapp2/commands/ghost', {})).status
  ).toBe(404);
});

test('command route is 404 in production', async () => {
  expect(
    (await prodPost('/api/v1/apps/anything/commands/build', {})).status
  ).toBe(404);
});
...
test('a port-only manifest with an action command but no workingDirectory 400s instead of spawning with cwd undefined', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: 'portonly',
      port: 4890,
      commands: { build: 'echo hi' },
    })
  );
  await devPost('/api/v1/apps/register', { dir });
  expect(
    (await devPost('/api/v1/apps/portonly/commands/build', {})).status
  ).toBe(400);
});
...
test("status carries a user app's command names in both dev and prod", async () => {
  ...
  writeFileSync(
    process.env.LOCAL_APPS_ROUTES_PATH!,
    JSON.stringify([{ hostname: 'metaapp.localhost', port: 4950, pid: 0 }])
  );
  const devRow = (await (await devApi('/api/v1/status')).json()).apps.find(
    (a: any) => a.name === 'metaapp'
  );
  expect(devRow.commands).toEqual(['deploy']);
  const prodRow = (await (await api('/api/v1/status')).json()).apps.find(
    (a: any) => a.name === 'metaapp'
  );
...
test('command route spawns in sourceDirectory when set, else workingDirectory', async () => {
  ...
  putRecord({
    ...
    commands: { mark: 'touch marker' },
  });
  const res = await devPost('/api/v1/apps/deck/commands/mark', {});
  expect(res.status).toBe(200);
  for (let i = 0; i < 40 && !existsSync(join(source, 'marker')); i++)
...
describe('command route gating by app class', () => {
  ...
  test('grandfathered managed row keeps dev-gated record.commands', async () => {
    ...
    for (let i = 0; i < 40 && !existsSync(join(source, 'built')); i++)
      await new Promise(r => setTimeout(r, 50));
  ...
  test('slim managed row 404s in prod, when unlinked, and on a broken link', async () => {
    ...
    writeFileSync(
      join(linkedDir, 'mattstack.deck.json'),
      JSON.stringify({
        name: 'slimapp',
        commands: {},
        dev: { build: 'touch built' },
      })
    );
    putRecord({
      name: 'slimapp',
    ...
    expect(
      (await prodPost('/api/v1/apps/slimapp/commands/build', {})).status
    ).toBe(404);
    ...
    expect(
      (await devPost('/api/v1/apps/slimapp2/commands/build', {})).status
    ).toBe(404);
    ...
    expect(
      (await devPost('/api/v1/apps/slimapp3/commands/build', {})).status
    ).toBe(404);
  });
  ...
  test('slim managed row 404s dev.start: the supervised job already owns that boot command', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        name: 'startapp',
        dev: { start: 'bun run serve', build: 'touch built' },
      })
    );
    putRecord({
      name: 'startapp',
    ...
    expect(
      (await devPost('/api/v1/apps/startapp/commands/start', {})).status
    ).toBe(404);
    // Sibling dev keys on the same manifest still run: only start is excluded.
    expect(
      (await devPost('/api/v1/apps/startapp/commands/build', {})).status
    ).toBe(200);
  });
  ...
  test('slim managed row runs a live-read dev key when linked in dev mode', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        name: 'liveapp',
        commands: {},
        dev: { build: 'touch marker2' },
      })
    );
    const second = await devPost('/api/v1/apps/liveapp/commands/build', {});
    expect(second.status).toBe(200);
    for (let i = 0; i < 40 && !existsSync(join(dir, 'marker2')); i++)
      await new Promise(r => setTimeout(r, 50));
  ...
  test("a run's status stays readable after its manifest key is deleted mid-run", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        name: 'midrunapp',
        commands: {},
        dev: { build: 'sleep 0.2 && touch built' },
      })
    );
    putRecord({
      name: 'midrunapp',
    ...
    writeFileSync(
      manifestPath,
      JSON.stringify({ name: 'midrunapp', commands: {}, dev: {} })
    );

    const status = await devApi(
      `/api/v1/apps/midrunapp/commands/build/${runId}`
    );
    expect(status.status).toBe(200);
    expect(['running', 'exited']).toContain((await status.json()).status);

    for (let i = 0; i < 40 && !existsSync(join(dir, 'built')); i++)
      await new Promise(r => setTimeout(r, 50));
  ...
  test("a run's status stays readable after its link breaks mid-run", async () => {
    ...
    const started = await devPost(
      '/api/v1/apps/breaklinkapp/commands/build',
      {}
    );
    expect(started.status).toBe(200);
    const runId = (await started.json()).runId as string;
    ...
    const status = await devApi(
      `/api/v1/apps/breaklinkapp/commands/build/${runId}`
    );
    expect(status.status).toBe(200);
    expect(['running', 'exited']).toContain((await status.json()).status);
  });
  ...
  test('an unknown runId still 404s with the same body in dev and in production', async () => {
    ...
    const prodStatus = await api(
      '/api/v1/apps/unknownrunapp/commands/build/deadbeefdeadbeef'
    );
    expect(prodStatus.status).toBe(404);
    expect(await prodStatus.json()).toEqual({ error: 'unknown run' });

    const devStatus = await devApi(
      '/api/v1/apps/unknownrunapp/commands/build/deadbeefdeadbeef'
    );
  ...
  test("grandfathered managed row's run status is readable after starting in dev", async () => {
    ...
    const started = await devPost(
      '/api/v1/apps/gfstatusapp/commands/build',
      {}
    );
    expect(started.status).toBe(200);
    const runId = (await started.json()).runId as string;
    ...
    const status = await devApi(
      `/api/v1/apps/gfstatusapp/commands/build/${runId}`
    );
    expect(status.status).toBe(200);
    expect(['running', 'exited']).toContain((await status.json()).status);
  });
