...
export async function registerApp(
  ...
  if (isService && (!input.command?.length || !input.workingDirectory)) {
    return {
      status: 400,
      body: { error: 'command + workingDirectory, or staticPort, required' },
    };
  }
  ...
  const taken =
    ...
    (!input.adopt &&
      routes.some(
        r => bareName(r.hostname, getPlatformSettings().tlds) === name
      ));
  ...
  if (port === undefined) {
    ...
    if (allocated === null)
      return { status: 507, body: { error: 'port range exhausted' } };
    port = allocated;
    ...
    if (isService && !notServedHere(record, serveShapeDeps)) {
      ...
      if (shape)
        await tryDriver(name, 'launchd', () =>
          drivers.manager.install(specFor(record, shape))
        );
    }
    await tryDriver(name, 'portless', () =>
      drivers.edge.alias(name, record.port)
    );
  }
...
async function flipRemoteBack(
  ...
  if (!drivers.railway || !drivers.dns) {
    throw new Error(
      `remote drivers missing: cannot remove remote app ${record.name}`
    );
  }
  const flip = await disableRemote(record.name, {
    railway: drivers.railway,
    dns: drivers.dns,
  });
...
export async function editApp(
  ...
  const devOnly =
    ...
    Object.prototype.hasOwnProperty.call(patch, 'dev');
  if (patch.dev != null) {
    const dir = patch.dev.workingDirectory;
    if (typeof dir !== 'string' || !dir.startsWith('/')) {
      return {
        status: 400,
        body: { error: 'dev.workingDirectory must be an absolute path' },
      };
    }
    if (!existsSync(dir))
    ...
    if (parsed === null)
      return {
        status: 400,
        body: { error: `no mattstack.deck.json in ${dir}` },
      };
  ...
  if (
    ...
    (!Number.isInteger(patch.port) || patch.port < 1 || patch.port > 65535)
  ) {
  ...
  if (servedHere && !serveShape(next, serveShapeDeps)) {
    return {
      status: 400,
      body: {
        error: `edit would leave ${next.name} with no runnable shape (no bundle, no valid source)`,
      },
    };
  }
  ...
  if (servedHere) {
    ...
    if (shape)
      await tryDriver(next.name, 'launchd', () =>
        drivers.manager.install(specFor(next, shape))
      );
  }
...
export async function adoptApp(
  ...
  if (!NAME_RE.test(target))
    return { status: 400, body: { error: 'bad name' } };
  if (managedBy === 'user')
    return { status: 400, body: { error: 'bad managedBy' } };
  ...
  if (renaming && getRecord(target))
    return { status: 409, body: { error: 'name taken', name: target } };
