...
test('remove on a managed record prints the escape hatch and exits 1', async () => {
  ...
  await fetch(`http://127.0.0.1:${PORT}/api/v1/apps/t-rt?force=true`, {
    method: 'DELETE',
  });
...
test('restart --managed / remove --managed only touch non-user records', async () => {
  ...
  const a = io();
  await runCommand(
    ['add', 't-managed', '--cmd', join(helpers, 't-managed'), '--dir', dir],
    a
  );
  // Flip it to rt-managed the same way the "escape hatch" test above does.
  await fetch(`http://127.0.0.1:${PORT}/api/v1/apps/t-managed?force=true`, {
    method: 'DELETE',
  });
  await fetch(`http://127.0.0.1:${PORT}/api/v1/apps`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      name: 't-managed',
      command: [join(helpers, 't-managed')],
...
test('usage text invokes the binary as deck (the shell-reserved-word rename), never local', async () => {
  ...
  expect(out).not.toMatch(
    /\blocal (status|add|remove|restart|logs|override|publish|password|access|domain|migrate|serve|setup|uninstall|update|version)\b/
  );
});
...
test('domain verb no longer prompts for or persists a CF token; it points at rt secrets set instead', async () => {
  const promptFn = () => {
    throw new Error('domain must not prompt for a CF token anymore');
  };
  const d = io();
...
test('deck access off succeeds against a registered app', async () => {
  const a = io();
  expect(await runCommand(['add', 't-access-off', '--port', '4997'], a)).toBe(
    0
  );
  const x = io();
...
test('deck access emails and domains reach the API with a well-formed payload', async () => {
  ...
  const e = io();
  const emailsCode = await runCommand(
    ['access', 't-access-on', 'emails', 'a@x.dev,b@y.dev'],
    e
  );
  ...
  const d = io();
  const domainsCode = await runCommand(
    ['access', 't-access-on', 'domains', 'corp.com,other.dev'],
    d
  );
...
test('deck access rejects a mode it does not know without calling the API', async () => {
  ...
  const code = await runCommand(
    ['access', 'no-such-app', 'only-me', '--email', 'm@x.dev'],
    x
  );
  expect(code).toBe(2);
  const out = x.lines.join('\n');
...
test('adopt --json: rename + ownership flip, idempotent re-run, frozen error strings', async () => {
  ...
  const first = io();
  expect(
    await runCommand(['adopt', 't-mrs', '--as', 't-board', '--json'], first)
  ).toBe(0);
  ...
  const rerun = io();
  expect(
    await runCommand(['adopt', 't-mrs', '--as', 't-board', '--json'], rerun)
  ).toBe(0);
...
test('deck cmd runs a declared action command', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: 'cmdcli',
      port: 4900,
      commands: { start: 's', build: 'echo hi' },
    })
  );
  expect(await runCommand(['register', '--dir', appDir], io())).toBe(0);
  const a = io();
