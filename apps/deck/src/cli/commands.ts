    ...
    switch (verb) {
      ...
      case 'list': {
        ...
        for (const row of body.apps ?? []) {
          ...
          const issues = (row.issues ?? [])
            .map((i: any) => ` !${i.source}`)
            .join('');
          const origin = row.remote
            ? ` [public:railway/${row.remote.status}]`
            : '';
          io.out(
            `${row.name.padEnd(24)} ${String(row.port ?? '-').padEnd(6)} ${health.padEnd(5)} ${managed}${issues}${origin}`
          );
        }
        return 0;
      }
      case 'url': {
        ...
        if (wantPublic) {
          ...
          if (!row?.published) {
            io.err(`${name} is not published; run \`deck publish ${name} on\``);
            return 1;
          }
          if (!row.publicUrl) {
            io.err(`${name} has no public url`);
            return 1;
          }
      ...
      case 'add': {
        ...
        const payload = port
          ...
          : {
              name,
              command: (cmd ?? '').split(/\s+/).filter(Boolean),
              workingDirectory: dir,
            };
        const { status, body } = await apiJson('/api/v1/apps', {
          method: 'POST',
      ...
      case 'remove': {
        if (rest.includes('--managed')) {
          ...
          for (const name of body.failed ?? [])
            io.err(`partly removed ${name} — the record was kept`);
        ...
        const { status, body } = await apiJson(`/api/v1/apps/${name}${force}`, {
          method: 'DELETE',
        });
        if (status !== 200) {
          io.err(body.message || body.error || `failed (${status})`);
        ...
        if (body.ok === false) {
          ...
          const detail = issues
            ...
            .join('; ');
          io.err(
            `partly removed ${name} — the record was kept${detail ? ` (${detail})` : ''}`
          );
          return 1;
        }
        io.out(`removed ${name}`);
      ...
      case 'restart': {
        if (rest.includes('--managed')) {
          ...
          for (const f of (body.failed ?? []) as Array<{
            name: string;
            error: string;
          }>)
            io.err(`failed to restart ${f.name} — ${f.error}`);
          return body.ok === false ? 1 : 0;
        }
        const [name] = rest;
        if (!name) {
        ...
        const { status, body } = await apiJson(`/api/v1/apps/${name}/restart`, {
          method: 'POST',
        });
      ...
      case 'logs': {
        ...
        const { body } = await apiJson(
          `/api/v1/apps/${name}/logs?lines=${lines}`
        );
      ...
      case 'override': {
        ...
        const { status, body } = await apiJson(
          `/api/v1/apps/${name}/override`,
          {
            method: 'PUT',
            body: JSON.stringify({ devPort }),
          }
        );
        if (status !== 200) {
          io.err(body.error ?? `failed (${status})`);
        ...
        io.out(
          port === 'off'
            ? 'override cleared'
            : `\`${name}.localhost\` now serves port ${port}`
        );
        return 0;
      }
      case 'publish': {
        ...
        const { status, body } = await apiJson(`/api/v1/apps/${name}/publish`, {
          method: 'PUT',
          body: JSON.stringify({ published: onOff === 'on' }),
        });
        if (status !== 200) {
          io.err(body.error ?? `failed (${status})`);
          return 1;
        ...
        io.out(
          onOff === 'on' ? `${name} is now public` : `${name} is now private`
        );
        return 0;
      }
      case 'password': {
      ...
      case 'access': {
        ...
        if (mode === 'off') {
          ...
          const items = (value ?? '')
            ...
            .filter(Boolean);
          if (!items.length) {
            io.err(`${mode} needs a comma-separated list`);
            return 2;
          }
          payload =
            mode === 'emails'
        ...
        if (body.cfSynced === false)
          io.err(
            "warning: Cloudflare sync failed, see the app's row on the board"
          );
        io.out(
          mode === 'off'
            ? 'google sign-in off'
            : `google sign-in limited to ${mode}`
        );
        return 0;
      }
      case 'adopt': {
        ...
        if (status !== 200) {
          if (asJson) {
            io.out(
              JSON.stringify({
                adopted: false,
                error: String(body.error ?? `failed (${status})`),
              })
            );
            return 1;
          }
      ...
      case 'alt': {
        ...
        io.out(
          which === 'off'
            ? `${name} back on its base config`
            : `${name} now on alt "${which}"`
        );
        return 0;
      }
      case 'cmd': {
        ...
        const { status, body } = await apiJson(
          `/api/v1/apps/${app}/commands/${name}`,
          { method: 'POST' }
        );
        if (status === 404) {
          io.err(`no such command (is deck in dev mode?): ${app} ${name}`);
          return 1;
        }
        if (status === 409) {
          io.err(`${app} is already running a command`);
          return 1;
        }
        if (status !== 200) {
          io.err(body.error ?? `failed (${status})`);
      ...
      case 'domain': {
        ...
        if (!arg) {
          ...
          if (!body.tunnel && !body.domain) {
            io.out('no edge bound. bind one with: deck domain <domain>');
            return 0;
          }
          io.out(
            `domain: ${body.domain ?? '(none: partial bind, run deck domain <domain> or deck domain unbind)'}`
          );
          if (body.tunnel)
            io.out(`tunnel: ${body.tunnel.name} (${body.tunnel.uuid})`);
          if (body.edge)
            io.out(
              `edge: ${body.edge.state} (${body.edge.detail})${body.edge.hint ? ` ... ${body.edge.hint}` : ''}`
            );
          return 0;
        }
        if (arg === 'unbind') {
          const { status, body } = await apiJson('/api/v1/domain/unbind', {
            method: 'POST',
            body: JSON.stringify({ force }),
          });
          if (status === 409 && body.error === 'apps-will-go-offline') {
            io.err(
              `unbinding takes these apps offline: ${body.apps.join(', ')}`
            );
          ...
          if (status === 409) {
            io.err(
              `${body.error}: ${(body.apps ?? []).join(', ')} (re-run with --force)`
            );
            return 1;
          }
          if (status !== 200) {
          ...
          io.out(
            'unbound: the tunnel, its DNS record and launchd service are gone'
          );
          return 0;
        }
        io.out(
          'DNS writes need a Cloudflare token with Zone.DNS:Edit: rt secrets set deck cfDnsToken (and: rt secrets set deck cfZoneId)'
        );
        const { status, body } = await apiJson('/api/v1/domain/bind', {
          method: 'POST',
          body: JSON.stringify({ domain: arg, force }),
        });
        if (status === 428) {
          io.err(`One step first: run \`${body.command}\`, then re-run this.`);
          return 1;
        }
        if (status === 409) {
          io.err(
            `${body.error}: ${(body.apps ?? []).join(', ')} (re-run with --force to rebind anyway)`
          );
          return 1;
        }
        if (status !== 200) {
          io.err(
            `${body.error ?? `failed (${status})`}${body.hint ? ` (${body.hint})` : ''}`
          );
          return 1;
        }
        const connectors =
          body.connectors > 0
            ? `${body.connectors} connector${body.connectors === 1 ? '' : 's'}`
            : 'connector still starting, check deck domain in a moment';
        io.out(
          `bound ${body.domain} via ${body.tunnel.name} (${connectors}) ... every published app is now https://<name>.${body.domain}`
        );
        return 0;
      }
      case 'remote': {
        const [app, onOff] = rest;
        if (!app || (onOff !== 'on' && onOff !== 'off')) {
          io.err(USAGE);
          return 2;
        }
        const { status, body } = await apiJson(`/api/v1/apps/${app}/remote`, {
          method: 'POST',
          body: JSON.stringify({ enabled: onOff === 'on' }),
        });
        if (status === 428) {
          io.err(
            'Railway needs a token... store with: rt secrets set deck railwayToken'
          );
          return 1;
        }
        if (status !== 200) {
        ...
        if (onOff === 'off') {
          io.out(`${app} back on the tunnel (public: tunnel)`);
          return 0;
        }
        io.out(
          `${app} now served remotely: ${body.url} (cutover: ${body.cutover})`
        );
        return 0;
      }
      case 'push': {
        ...
        const { status, body } = await apiJson(`/api/v1/apps/${app}/push`, {
          method: 'POST',
        });
        if (status !== 200) {
          io.err(body.error ?? `failed (${status})`);
