...
function devPortNav(page: Page) {
  return page.locator('[data-part="listgroup-nav"]').filter({
    has: page.locator('[data-part="listgroup-label"]', {
      hasText: 'dev port',
    }),
  });
}
...
test("when the open row's data vanishes, the drawer closes and focus lands on a stable fallback", async () => {
  await withBoard(async page => {
...
test('root screens render per row kind: app (public+nav+actions+danger), service (reduced), tunnel (facts+restart)', async () => {
  await withBoard(async page => {
    ...
    expect(
      await drawer
        .locator('[data-part="listgroup-toggle"]', { hasText: 'public' })
        .count()
    ).toBe(1);
    ...
    expect(
      await drawer
        .locator('button[data-intent="bad"][data-centered]', {
          hasText: 'remove app',
        })
        .count()
    ).toBe(1);
    ...
    await openDrawer(page, 'stray-agent');
    expect(await drawer.locator('[data-part="listgroup-toggle"]').count()).toBe(
      0
    );
    expect(
      await drawer.locator('button[data-intent="bad"][data-centered]').count()
    ).toBe(0);
    expect(
      await drawer
    ...
    expect(
      await drawer
        .locator('[data-part="listgroup-fact"]', { hasText: 'carries' })
        .count()
    ).toBe(1);
    expect(
      await drawer
        .locator('[data-part="listgroup-action"]', {
          hasText: 'restart tunnel',
        })
        .count()
    ).toBe(1);
  });
...
test("a broken app's root shows an error banner and a bad-tone logs hint", async () => {
  await withBoard(async page => {
    ...
    const drawer = page.locator('[data-part="sidedrawer"]');
    expect(
      await drawer.locator('[data-part="alert"][data-intent="bad"]').count()
    ).toBe(1);
    expect(
      await drawer.locator('[data-part="listgroup-nav"] .t-bad').count()
    ).toBe(1);
  });
});
...
test('dev port: override-active screen shows assigned + override facts and the atlas footer copy', async () => {
  await withBoard(async page => {
    ...
    expect(await page.locator('[data-part="drawer-back"]').textContent()).toBe(
      '‹ orbit'
    );
    ...
    expect(
      await facts.nth(0).locator('[data-part="listgroup-label"]').textContent()
    ).toBe('assigned port');
    expect(
      await facts.nth(0).locator('[data-part="listgroup-value"]').textContent()
    ).toBe('11007');
    expect(
      await facts.nth(1).locator('[data-part="listgroup-label"]').textContent()
    ).toBe('override');
    expect(
      await facts.nth(1).locator('[data-part="listgroup-value"]').textContent()
    ).toBe('3007');
...
test('dev port: revert calls the override-clear mutation; the screen and the root hint update', async () => {
  await withBoard(async page => {
    ...
    expect(
      await page.locator('[data-part="listgroup-footer"]').textContent()
    ).toBe('orbit serves on its assigned port — no override');

    await page.locator('[data-part="drawer-back"]').click();
    expect(
      await devPortNav(page)
        .locator('[data-part="listgroup-value"]')
        .textContent()
    ).toBe('11007');
  });
});
...
test("dev port: the board's own row (self) offers no 'set override…' -- no dead-end into the setting screen", async () => {
  await withBoard(async page => {
    ...
    expect(
      await page
        .locator('[data-part="listgroup-fact"] [data-part="listgroup-value"]')
        .textContent()
    ).toBe('11003');
    expect(
      await page.locator('[data-part="listgroup-footer"]').textContent()
...
test("dev port: no override shows 'set override…'; editing has the input + nav save; cancel returns without saving", async () => {
  await withBoard(async page => {
    ...
    expect(
      await page.locator('[data-part="listgroup-footer"]').textContent()
    ).toBe('atlas serves on its assigned port — no override');
    ...
    await page
      .locator('[data-part="listgroup-action"] button', { hasText: 'cancel' })
      .click();
...
test('dev port: the kit back chevron out of the setting screen clears the draft, so refresh() is not frozen behind it', async () => {
  await withBoard(async page => {
    let statusHits = 0;
    await page.route('**/api/v1/status', async route => {
      statusHits++;
...
test('edit app: a user service row shows name, base port, command and directory, prefilled from the row', async () => {
  await withBoard(async page => {
    ...
    expect(
      await drawer.getByRole('textbox', { name: 'name' }).inputValue()
    ).toBe('orbit');
    expect(
      await drawer.getByRole('textbox', { name: 'base port' }).inputValue()
    ).toBe('11007'); // orbit overrides: base 11007, dev 3007
    expect(
      await drawer.getByRole('textbox', { name: 'command' }).inputValue()
    ).toBe('bun run dev');
...
test('edit app: a managed row offers no edit nav — the resolver owns its shape; source replaces it', async () => {
  await withBoard(async page => {
    ...
    expect(
      await drawer
        .locator('[data-part="listgroup-nav"] button', { hasText: 'source' })
        .count()
    ).toBe(1);
  });
});
...
test('source screen: facts, relink, and Unlink PATCHes dev:null', async () => {
  await withBoard(async page => {
    ...
    const drawer = page.locator('[data-part="sidedrawer"]');
    expect(
      await drawer.getByText('/Users/matt/Documents/GitHub/atlas').count()
    ).toBe(1);
    expect(await drawer.getByText('build · deploy').count()).toBe(1);

    await drawer
      .locator('[data-part="listgroup-action"] button', { hasText: 'Unlink' })
      .click();
...
test('edit app: an external row shows only name and base port', async () => {
  await withBoard(
    async page => {
      ...
      expect(await drawer.getByText('command', { exact: true }).count()).toBe(
        0
      );
      expect(await drawer.getByText('directory', { exact: true }).count()).toBe(
        0
      );
    },
...
test('edit app: save PATCHes the edit payload and pops back to root', async () => {
  await withBoard(async page => {
    ...
    await waitUntil(
      async () =>
        (await page.locator('[data-part="drawer-title"]').textContent()) ===
        'orbit'
    );
    expect(await page.locator('[data-part="drawer-back"]').count()).toBe(0);
  });
...
test('edit app: an API validation error renders inline on the name field; the screen stays open', async () => {
  await withBoard(async page => {
    ...
    const fieldError = page.locator(
      '[data-part="sidedrawer"] [data-part="field-error"]'
    );
...
test("remove: the danger row opens the ConfirmDialog with the atlas's blast-radius copy", async () => {
  await withBoard(async page => {
    ...
    expect(
      await dialog.locator('[data-part="modal-title"]').textContent()
    ).toBe('remove atlas?');
    expect(
      await dialog.locator('[data-part="confirmdialog-body"]').textContent()
    ).toBe(
...
test('remove: cancel closes the dialog without deleting; the drawer stays open', async () => {
  await withBoard(async page => {
    ...
    await page
      ...
      .click();
    await page
      .locator('[data-part="modal"] button', { hasText: 'cancel' })
      .click();
...
test('remove: confirm DELETEs the app and closes the drawer', async () => {
  await withBoard(async page => {
    let deleteCalled = false;
    await page.route('**/api/v1/apps/atlas', async route => {
