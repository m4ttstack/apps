...
function SettingsModal({
    ...
    (username: string) => {
      ...
      fetch('/peer/invite', {
        ...
        .then(async r => {
          ...
          setIssued({
            username: name,
            invite: (JSON.parse(text) as { invite: string }).invite,
          });
        })
        .catch(() => {
          setIssued(null);
  ...
  const joinRef = useRevealOnChange<HTMLDivElement>(
    joinOpen || peering === 'unauthorized'
  );
  ...
  return (
    ...
    >
      ...
      <ul className="tui-modal-list">
        {members.map(m => {
          const peerState =
            m.username === defaultMember || !canInvite
              ? 'unknown'
              : memberPeerState(m.username, peered);
          return (
            <li
              key={m.username}
            ...
            >
              <label
                className="tui-modal-name"
                title={
                  m.hidden
                    ? 'checked out -- hidden from the board'
                    : 'checked in'
                }
              >
                <input
      ...
      {canInvite && (
        <div className="tui-invite-new">
          ...
          <button
            className="tui-invite-btn"
            disabled={pending !== null || !handle.trim()}
            onClick={() => ask(handle)}
          >
            invite
          </button>
      ...
      {issued && (
        <div ref={issuedRef} className="tui-invite-row">
          ...
          <CopyButton
            ...
            label="copy invite"
          />
          <span className="tui-invite-note">
            for {issued.username} · one-time, expires in 7 days
          </span>
        </div>
      )}
      ...
      {local && (
        <div ref={joinRef} className="tui-join-row">
          {join.warning && (
            <p className="tui-modal-sub tui-join-warning">{join.warning}</p>
          )}
          {joinExpanded ? (
            <>
              ...
              {joinError && (
                <p
                  ref={joinErrorRef}
                  className="tui-modal-sub tui-invite-error"
                >
                  {joinError}
                </p>
              )}
