...
const commentedReview: ReviewState = {
  mrUrl: nudge.mrUrl,
  iid: 1,
  status: 'done',
  outcome: 'comment',
...
describe('decideNudge', () => {
  test('handled always skips, even if everything else would dispatch', () => {
    const handled: NudgeState = {
      ...nudge,
      handled: { at: NOW, result: 'launched' },
    };
    expect(decideNudge(handled, commentedReview, m, cfg, NOW)).toEqual({
      action: 'skip',
  ...
  test('in-flight own review rejects, outranking budget exhaustion', () => {
    ...
    expect(decideNudge(nudge, inFlight, exhausted, cfg, NOW)).toEqual({
      action: 'reject',
  ...
  test("no own review, or one that isn't a done+comment, rejects", () => {
    expect(decideNudge(nudge, undefined, m, cfg, NOW)).toEqual({
      action: 'reject',
      reason: 'no-commented-review',
    });
    const approved: ReviewState = { ...commentedReview, outcome: 'approve' };
    expect(decideNudge(nudge, approved, m, cfg, NOW)).toEqual({
      action: 'reject',
      reason: 'no-commented-review',
    });
  });
  ...
  test('budget exhausted rejects', () => {
    const exhausted: MrMemory = { ...m, attemptsToday: cfg.dailyAttemptBudget };
    expect(decideNudge(nudge, commentedReview, exhausted, cfg, NOW)).toEqual({
      action: 'reject',
  ...
  test('cooldown rejects', () => {
    ...
    expect(decideNudge(nudge, commentedReview, cooling, cfg, NOW)).toEqual({
      action: 'reject',
      reason: 'cooldown',
    });
    const cooled: MrMemory = { ...m, lastDispatchAt: NOW - 31 * 60_000 };
    expect(decideNudge(nudge, commentedReview, cooled, cfg, NOW).action).toBe(
      'dispatch'
    );
  });

  test('clean state dispatches', () => {
    expect(decideNudge(nudge, commentedReview, m, cfg, NOW)).toEqual({
      action: 'dispatch',
      reason: 'nudge',
    });
  });
...
describe('runNudgePass', () => {
  ...
  test('reject path marks handled with reason, publishes, notifies, audits, and does not touch memory counters', async () => {
    ...
    const d = deps({
      readReviewStates: () => new Map([[nudge.mrUrl, notCommented]]),
    });
    const result = await runNudgePass(d);
    expect(result).toEqual({
      dispatched: 0,
...
describe('decideRequest', () => {
  ...
  test('a latch request never expires, however old the MR is', () => {
    ...
    expect(
      decideRequest(latchReq, commentedReview, m, cfg, wayLater).action
    ).toBe('dispatch');
  });
