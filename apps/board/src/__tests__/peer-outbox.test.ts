...
const d = (id: string): DraftEnvelope => ({
  id,
  to: 'grace',
  type: 'review-state',
  sentAt: 1,
...
describe('makeSwitchboardClient', () => {
  ...
  test('inbox parses envelopes and returns null on failure', async () => {
    ...
    const good = (async () =>
      new Response(JSON.stringify({ envelopes: [env, { junk: true }] }), {
        status: 200,
      })) as unknown as typeof fetch;
    const envelopes = await makeSwitchboardClient(
      'https://sb.test',
      't',
      good
    ).inbox();
    expect(
      Array.isArray(envelopes) ? envelopes.map(e => e.id) : envelopes
    ).toEqual(['e']);
    const bad = (async () =>
      new Response('nope', { status: 500 })) as unknown as typeof fetch;
    expect(
      await makeSwitchboardClient('https://sb.test', 't', bad).inbox()
    ).toBeNull();
  });
  test('inbox distinguishes a 401 from any other relay failure', async () => {
    ...
    expect(
      await makeSwitchboardClient('https://sb.test', 't', denied).inbox()
    ).toBe('unauthorized');
  });
});
