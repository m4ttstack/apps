...
describe('displayName: stored name beats the GitLab profile', () => {
  test('a stored name wins over the GitLab profile name', () => {
    expect(
      displayName({ username: 'dcf417', name: 'Darryl Faint' }, 'D. Faint')
    ).toBe('Darryl Faint');
  });

  test('the GitLab profile fills in when there is no stored name', () => {
    expect(displayName({ username: 'edroch' }, 'Ed Rocha')).toBe('Ed Rocha');
  });

  test('null when neither side has one', () => {
    expect(displayName({ username: 'djclaims' }, null)).toBeNull();
    expect(displayName({ username: 'djclaims' }, undefined)).toBeNull();
  });
...
Tell Matt, in one line, that this write drops `leath1` (Leath Cooper), `hacknightly`, and `CalebDudley-Assured` from `mattstack.roster`, which is the roster boxscore's leaderboard reads, and that `leath1` and `hacknightly` are already in his `board.hiddenMembers` overlay. Wait for confirmation before Step 3. Do not skip this step: it is a shared team-store write that changes another app's behavior.
...
Paste both team-scope values into the task report. The team store is git-backed, so `git -C ~/.mattstack/teams/claimview log -1` also gives a restore point; record that SHA.
...
rt settings set mattstack.roster --scope team '[
  ...
  {"username":"edroch","name":"Ed Rocha"},
  {"username":"doug-at-assured","name":"Doug Treadwell"},
  {"username":"jorgecoello","name":"Jorge Coello"},
  {"username":"djclaims","name":"Djam Saidmuradov"},
  {"username":"dcf417","name":"Darryl Faint"}
]'
```
...
Then ask Matt to confirm in the running board that `djclaims` reads "Djam Saidmuradov" and `dcf417` reads "Darryl Faint", that the settings modal and the roster panel now agree on who is checked out, and that adding someone with a name and renaming someone both stick. Ask before driving a browser yourself: per the ask-before-browser-verification rule, Matt likely has the board open already.

- [ ] **Step 6: Note the team-store push**

`setSetting` prints the reminder itself: a team write stays local until committed and pushed from `~/.mattstack/teams/claimview`. Surface that to Matt rather than pushing on his behalf.
...
2. The `rosterSummary` / `hiddenSet` replace-not-union fix (Task 5 Steps 3-4). This is the bug behind `djclaims` reading "checked out" in the settings modal while checked-in in the roster panel. It is one line in each of two places and sits in exactly the code Task 5 already rewrites. If a reviewer wants it split out, it is cleanly separable.
